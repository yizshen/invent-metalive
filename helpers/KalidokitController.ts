...
export class KalidokitController {
    ...
    public productLoader = new GLTFLoader()
    ...
    public resetBackgroup(url: string) {
        const bgTexture = this.textureLoader.load(url, function (texture) {});
    ...
    public init = async (onProgress: (event: ProgressEvent<EventTarget>) => void) => {
        ...
        if(this.videoPath == null || this.videoPath == "") {
            ...
            this.video.src = "/zimeng.mp4"
            this.video.loop = true;
            this.video.controls = true;
            var videoFrameCallback = async () => {
                // videoElement.pause()
                await holistic.send({ image: this.video });
                this.video.requestVideoFrameCallback(videoFrameCallback);
                // videoElement.play()
            };
            this.video.requestVideoFrameCallback(videoFrameCallback);
            this.video.play()
    ...
    public loadProduct = (modelFile: string) => {
        if(this.mesh == null){
            this.productLoader.load(
               modelFile,
               (gltf) => {
                   // this.productObj = gltf
                   this.mesh = gltf.scene;
                   const animations = gltf.animations;
                   this.mesh.position.set(0.5, 1.5, 0);
                   this.mesh.rotateY(Math.PI / 3)
    ...
    public moveProduct = () => {
        ...
        if (this.mesh != null) {
            this.mesh.position.set(0.2, 1.8, 0);
